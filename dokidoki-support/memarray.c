...
static int type_error(lua_State *L, int n, const char *expected)
{
   const char *msg = lua_pushfstring(L, "%s expected, got %s", expected,
                                     luaL_typename(L, n));
...
LUALIB_API int luaopen_memarray(lua_State * L)
{
   ...
   luaL_setfuncs(L, reg_memarray_ptr, 0);
   lua_pop(L, 1);

   /* the module is a callable table, so that requiring it gives something
      which can be called to allocate an array; it acts as its own metatable */
   lua_newtable(L);
   lua_pushliteral(L, VERSION);
   lua_setfield(L, -2, "VERSION");
   luaL_setfuncs(L, reg_memarray, 0);
