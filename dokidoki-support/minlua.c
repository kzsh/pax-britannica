...
void init_preloaders(lua_State *L)
{
    ...
    // a game can register its own modules by building with -DEXTRA_LOADERS
    // and putting REGISTER_LOADER lines in an extra_loaders.h on the include
    // path
    #ifdef EXTRA_LOADERS
    #include "extra_loaders.h"
    #endif
...
// error handler for the main chunk, adds a traceback to the error message
static int add_traceback(lua_State *L)
{
...
int main(int argc, char ** argv)
{
    ...
    // run the app, with error reporting
    ...
    if(!error)
    {
        // pass the command-line arguments to the main chunk as well
        luaL_checkstack(L, argc - 1, "too many command line arguments");
        for(i = 1; i <= argc - 1; i++)
            lua_pushstring(L, argv[i]);
        error = lua_pcall(L, argc - 1, 0, handler_index);
    }
