...
local env = require "dokidoki.private.env"
...
function game_module.make_script(name, init)
  return {name = name, init = init}
end

local make_script = game_module.make_script
...
--- these variables.
function game_module.make_blueprint(name, ...)
  return {name = name, ...}
end

local make_blueprint = game_module.make_blueprint
...
function game_module.make_game(update_methods, draw_methods, init)
  local game = {}
  game.actors = {}
  ...
  function game.init_component(name)
    ...
    local old_env = env.get(component_init)
    env.set(component_init, setmetatable({}, {
    ...
    component_init(name)
    env.set(component_init, old_env)
  end
  ...
  function game.actors.new(blueprint, ...)
    ...
    for _, script_spec in ipairs(blueprint) do
      ...
      local old_env = env.get(script_init)
      env.set(script_init, setmetatable({}, {
        __index = function (_, k)
  ...
  local function update (dt)
    ...
    for k, _ in pairs(scripts_by_method) do
      scripts_by_method[k] =
        base.ifilter(function (s) return not s.self.dead end,
                     scripts_by_method[k])
    end
    for k, _ in pairs(actors_by_tag) do
      actors_by_tag[k] =
        base.ifilter(function (a) return not a.dead end, actors_by_tag[k])
    end
  end
